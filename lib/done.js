...
var parseFile = function(filename, fd, filelink, callback) {
    ...
    if (config['custom'].mathjax) {
        ...
        content = content
            ...
            .replace(/"/g, '&quot;')
            .replace(/'/g, '&#39;');
        mjParse(content, function(result) {
            var r = result.replace(/^&gt;/mg, '>');
            r = config['custom'].autospacing ? pangunode(marked(r, { renderer: renderer })) : marked(r, { renderer: renderer });
            callback({
                title: title,
                category: category,
                link: filelink,
        ...
        content = config['custom'].autospacing ? pangunode(marked(content, { renderer: renderer })) : marked(content, { renderer: renderer });
        callback({
            title: title,
            category: category,
            link: filelink,
...
var lastUpdate = function(file, content) {
    ...
    var sourcepath = file.replace(path.resolve(cwd + '/' + config['dir'].source), config['dir'].raw);

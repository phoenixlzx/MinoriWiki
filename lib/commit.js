    ...
    exec(cmdPrefix + 'commit -m "' + msg + '"', function (err) {
        ...
        exec(cmdPrefix + 'push -f ' + config['deploy'].repo + ' master:' + config['deploy'].branch, function (err) {
            if (err) {
                ...
                console.log('[Git] Site updated.');
            }
        });
        // use enabled: false to disable the other deployments, otherwise we just deploy them one by one.
        // deploy to AWS.
        if (config['deploy'].s3 && config['deploy'].s3.enabled) {
            deploy_s3();
        }
        // manual deploy command
        if (config['deploy'].pushcommand && config['deploy'].pushcommand.enabled) {
            pushcommand(config['deploy'].pushcommand.command);
        }
    });
};
...
function pushcommand(cmd) {
    console.log('Executing deploy command:', cmd);
    exec(cmd, function (err) {
        if (err) {
            console.error('Unable to push changes:', err);
            process.exit(1);
        } else {
            console.log('[PushCommand] Site updated.');
        }
    });
}
